...
class Renderer
{
	...
	GW::MATH::GVECTORF lightDir = /*{ (-1.0f/sqrt(6.0f)), (-1.0f / sqrt(6.0f)), (2.0f / sqrt(6.0f)) }*/{ -1.0f, -1.0f, 2.0f }; //original values were { -1.0f, -1.0f, 2.0f }
...
public:
	void Render()
	{
		...
		for (int i = 0; i < FSLogo_meshcount; i++)
		{
			vkCmdDrawIndexed(commandBuffer, FSLogo_meshes[i].indexCount, 1, FSLogo_meshes[i].indexOffset, 0, FSLogo_meshes[i].materialIndex);
		}
